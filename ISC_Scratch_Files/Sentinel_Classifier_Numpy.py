...
of the classificed map and albedo map. Then, the cloud mask is produced from the cloud probability map provided as part
...
*** Note on xarray ***
xarray is used in this script to transfer files to and from netcdf storage and to maintain data labelling. An alternative
version of this script was developed using xarray much more extensively (i.e. keeping all datasets inside xarray rather
than performing operations after extracting arrays to numpy and then returning the altered array back to xarray).
However, testing showed a major increase in memory utilization and reduction in speed with the xarray version, which was
surprising given the out-of-memory utility of xarray. This turned out to be due to a severe spike in memory utilization
in the xr.unstack function discussed in xarray issue 1560. Updating xarray to version 0.11.3 and pandas to version 2.4.1
did not resolve the issue. I have therefore continued developing this code with arrays transferred between xarray and
numpy as necessary, understanding that this necessitates running the script on a fairly powerful machine with >32GB RAM.

"""
# TODO: Try loading jp2s into numpy using rasterio instead of additional out-of-script gdal steps
# TODO: organise paths for easier iteration
...
def set_paths(virtual_machine = False):
    ...
    if not virtual_machine:
        ...
        Sentinel_template = str(mask_path+'MaskTemplate_KGR.jp2')
    ...
    return hcrf_file, savefig_path, img_path, Sentinel_template, Icemask_in, Icemask_out, cloudmaskpath, cloudProbThreshold


def create_dataset(hcrf_file, img_path, save_spectra_plots=True):
# Sentinel 2 dataset
...
def split_train_test(X, test_size=0.2, n_trees= 64, print_conf_mx = True, save_confmx_figs = False,
    ...
    Y_predict_RF_train = clf.predict(X_train)
    conf_mx_RF_train = confusion_matrix(Y_train, Y_predict_RF_train)
    recall_RF_train = recall_score(Y_train, Y_predict_RF_train, average="weighted")
    f1_RF_train = f1_score(Y_train, Y_predict_RF_train, average="weighted")
    precision_RF_train = precision_score(Y_train, Y_predict_RF_train, average='weighted')
    ...
    conf_mx_RF = confusion_matrix(Y_test, Y_predict_RF)
    ...
    if save_confmx_figs:
        ...
        ax2.set_title('Normalised Confusion Matrix: TRAINING SET'), ax2.set_aspect('equal')
        plt.tight_layout()
    ...
    if print_conf_mx:
        print('Final Confusion Matrix: TRAINING SET')
        print(conf_mx_RF_train)
        print()
        print('Normalised Confusion Matrix: TRAINING SET')
        print(norm_conf_mx_train)
        print('Final Confusion Matrix: TEST SET')
        print(conf_mx_RF)
        print()
        print('Normalised Confusion Matrix: TEST SET')
        print(norm_conf_mx)
...
def format_mask (Sentinel_template, Icemask_in, Icemask_out, cloudmaskpath, cloudProbThreshold):
    """
...
hcrf_file, savefig_path, img_path, Sentinel_template, mask_in, mask_out, cloudmaskpath, cloudProbThreshold = set_paths(virtual_machine=False)

#create dataset
S2vals, X = create_dataset(hcrf_file, img_path, save_spectra_plots=False)
...
clf = split_train_test(X, test_size=0.2, n_trees= 32, print_conf_mx = False,
                                                 save_confmx_figs = False, show_model_performance = True,
                                                 pickle_model=False)
# apply model to Sentinel2 image
predicted, albedo, dataset =  ClassifyImages(S2vals, clf, icemask, cloudmask, savefigs=False, save_netcdf=False)
